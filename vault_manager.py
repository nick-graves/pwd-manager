...
def initialize(count):
    ...
    if not os.path.exists(KEYFILE):
        ...
        return _verify_master_password(count)
...
def _verify_master_password(count):
    count += 1
...
def check_id(cred_id):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT id FROM credentials")
    rows = cursor.fetchall()
